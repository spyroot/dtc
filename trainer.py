...
class Trainer:
    ...
    def __init__(self, experiment_specs: ExperimentSpecs, dataloader):
        """
        ...
        self.train_loader, self.val_set, self.collate_fn = dataloader.create()
    ...
    def load_model(self):
        ...
        :return:
        """
        model = Tacotron2(self.experiment_specs).cpu()
    ...
    def train(self):
        ...
        optimizer = torch.optim.Adam(model.parameters(),
                                     lr=learning_rate,
                                     weight_decay=self.experiment_specs.weight_decay)
        #
        ...
        # is_overflow = False
        # # ================ MAIN TRAINNIG LOOP! ===================
        for epoch in range(epoch_offset, 10):
            ...
            for i, batch in enumerate(self.train_loader):
                ...
                if self.experiment_specs.is_fp16_run():
                    grad_norm = clip_grad_norm_(amp.master_params(optimizer), self.experiment_specs.grad_clip_thresh)
...
if __name__ == '__main__':
    ...
    fmtl_print("active model", experiment_specs.active_model)
    fmtl_print("active dataset", experiment_specs.use_dataset)

    model_spec = experiment_specs.get_model_spec()
    dataloader = Mel_Dataloader(experiment_specs)
    trainer = Trainer(experiment_specs, dataloader)
    trainer.train()
